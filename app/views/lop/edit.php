...
<div class="container">
    ...
    <div class="card-form-premium">
        <div class="d-flex align-items-center justify-content-between mb-4 border-bottom pb-3">
            <h2 class="h4 fw-bold mb-0 text-slate-800"><i class="fa-solid fa-pen-to-square text-primary me-2"></i> Chỉnh sửa thông tin lớp học</h2>
        ...
        <form action="/lop/update/<?php echo $lop['STT']; ?>" method="POST">
            <!-- Mã lớp -->
            <div class="mb-3">
                <label for="MaLop" class="form-label form-label-premium">Mã lớp học</label>
                <input type="text" id="MaLop" name="MaLop" class="form-control form-control-premium" placeholder="Ví dụ: CNTT02, KTPM01" value="<?php echo htmlspecialchars($lop['MaLop']); ?>" required>
                <small class="form-text text-muted">Lưu ý: Thay đổi mã lớp sẽ ảnh hưởng đến các sinh viên đang liên kết với mã lớp này.</small>
            </div>

            <!-- Tên lớp -->
            <div class="mb-3">
                <label for="TenLop" class="form-label form-label-premium">Tên lớp học</label>
                <input type="text" id="TenLop" name="TenLop" class="form-control form-control-premium" placeholder="Ví dụ: Công nghệ Thông tin 2" value="<?php echo htmlspecialchars($lop['TenLop']); ?>" required>
            </div>

            <!-- Ghi chú -->
            <div class="mb-4">
                <label for="GhiChu" class="form-label form-label-premium">Ghi chú</label>
                <textarea id="GhiChu" name="GhiChu" class="form-control form-control-premium" rows="3" placeholder="Nhập ghi chú hoặc thông tin bổ sung (nếu có)"><?php echo htmlspecialchars($lop['GhiChu'] ?? ''); ?></textarea>
            </div>
